...
exports.rewritePathLeaf = rewritePathLeaf;
exports.readProtobufField = readProtobufField;
exports.readProtobufString = readProtobufString;
exports.rewriteProtobuf = rewriteProtobuf;
 ...
 */
function rewritePathLeaf(str, fromStr, toStr) {
    const rewritten = (0, paths_1.rewritePaths)(str, fromStr, toStr, { jsonEscaped: false });
    if (rewritten === str || !rewritten.includes("file://"))
        return rewritten;
...
function primitiveFieldSize(wireType, buffer, offset) {
    if (wireType === 0) {
        // varint
        let bytes = 0;
        while (offset + bytes < buffer.length) {
 ...
 */
function readProtobufField(buffer, fieldPath) {
    let cur = buffer;
...
function readProtobufString(buffer, fieldPath) {
...
function rewriteProtobuf(buffer, fromStr, toStr) {
